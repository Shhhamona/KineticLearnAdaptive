...
def run_adaptive_zone_experiment(uniform_dataset, test_dataset,
    ...
    # Define fixed validation set from last 500 samples (consistent across all iterations)
    eval_sample_size = min(500, len(batch_new_x_all))
    if eval_sample_size <= 0:
        print(f"   ❌ No batch samples available!")
        return iteration_results
        
    eval_start_idx = len(batch_new_x_all) - eval_sample_size
    eval_indices = set(range(eval_start_idx, len(batch_new_x_all)))
    print(f"   🔒 Fixed validation set: last {eval_sample_size} samples (indices {eval_start_idx}-{len(batch_new_x_all)-1})")
    
    # Add validation indices to used_batch_indices so they won't be sampled for training
    used_batch_indices.update(eval_indices)
    
    eval_x = batch_new_x_all[list(eval_indices)]
    eval_y_scaled = batch_new_y_scaled_all[list(eval_indices)]
...
if __name__ == '__main__':
    ...
    batch_json_files = [batch_json_path_1, batch_json_path_2, batch_json_path_3]
    ...
    batch_k_values, batch_compositions, n_simulations, n_pressure_conditions = load_batch_data(batch_json_path_1)
    ...
    seed_list = [40, 41, 42, 43, 44, 45, 46, 47, 48]

    adaptive_result_list = []
    random_results_list = []
    ...
    for seed_shuffle in seed_list:
        ...
        batch_new_x_all, batch_new_y_scaled_all = shuffle(batch_new_x_all, batch_new_y_scaled_all, random_state= seed_shuffle)
        ...
        adaptive_result_list.append(adaptive_results)
        random_results_list.append(random_results)
    
    # ----------------------------
    # PHASE 4: Aggregate results across seeds and create comparison
    # ----------------------------
    print(f"\n📊 PHASE 4: Aggregate results across seeds ({len(adaptive_result_list)} seeds)")

    def average_results_across_seeds(results_per_seed_list):
        """Average a list of per-seed result-lists into a single averaged result-list.

        Each element in results_per_seed_list is itself a list of iteration dicts.
        This function averages numeric fields (total_mse, mse_per_output, zone lists) per iteration
        and returns a list of averaged iteration dicts.
        """
        if not results_per_seed_list:
            return []

        # Determine max iterations across seeds
        n_iters = max(len(r) for r in results_per_seed_list)
        averaged = []

        for it_idx in range(n_iters):
            # collect all seeds that have this iteration
            seed_dicts = [seed[it_idx] for seed in results_per_seed_list if len(seed) > it_idx]
            if not seed_dicts:
                continue

            # Basic fields
            total_samples_vals = [d.get('total_samples') for d in seed_dicts if d.get('total_samples') is not None]
            batch_added_vals = [d.get('batch_samples_added') for d in seed_dicts if d.get('batch_samples_added') is not None]

            # MSE per output: list of lists
            mse_per_output_vals = [np.array(d.get('mse_per_output'), dtype=float) for d in seed_dicts if d.get('mse_per_output') is not None]
            if mse_per_output_vals:
                mse_per_output_stack = np.vstack(mse_per_output_vals)
                mse_per_output_mean = np.nanmean(mse_per_output_stack, axis=0).tolist()
            else:
                mse_per_output_mean = None

            # Total MSE
            total_mse_vals = [d.get('total_mse') for d in seed_dicts if d.get('total_mse') is not None]
            total_mse_mean = float(np.nanmean(total_mse_vals)) if total_mse_vals else None

            # Zone-wise overall MSE (list per dict)
            zone_lists = [d.get('zone_overall_mse_scaled') for d in seed_dicts if d.get('zone_overall_mse_scaled')]
            # Determine max zones
            max_zones = 0
            for zl in zone_lists:
                if zl and isinstance(zl, (list, tuple, np.ndarray)):
                    max_zones = max(max_zones, len(zl))

            if max_zones > 0:
                zone_matrix = np.full((len(zone_lists), max_zones), np.nan)
                for i, zl in enumerate(zone_lists):
                    for z in range(min(max_zones, len(zl))):
                        try:
                            zone_matrix[i, z] = zl[z]
                        except Exception:
                            zone_matrix[i, z] = np.nan
                zone_mean_list = [float(x) if not np.isnan(x) else None for x in np.nanmean(zone_matrix, axis=0)]
            else:
                zone_mean_list = None

            # Zone counts
            zone_counts_lists = [d.get('zone_counts') for d in seed_dicts if d.get('zone_counts')]
            if zone_counts_lists:
                max_z = max(len(zc) for zc in zone_counts_lists)
                zc_matrix = np.full((len(zone_counts_lists), max_z), np.nan)
                for i, zc in enumerate(zone_counts_lists):
                    for z in range(len(zc)):
                        try:
                            zc_matrix[i, z] = zc[z]
                        except Exception:
                            zc_matrix[i, z] = np.nan
                zc_mean = [int(np.nanmean(zc_matrix[:, z])) if not np.isnan(np.nanmean(zc_matrix[:, z])) else 0 for z in range(max_z)]
            else:
                zc_mean = None

            averaged.append({
                'iteration': it_idx,
                'total_samples': int(np.round(np.nanmean(total_samples_vals))) if total_samples_vals else None,
                'batch_samples_added': int(np.round(np.nanmean(batch_added_vals))) if batch_added_vals else None,
                'mse_per_output': mse_per_output_mean,
                'total_mse': total_mse_mean,
                'mse_type': seed_dicts[0].get('mse_type'),
                'sampling_strategy': seed_dicts[0].get('sampling_strategy'),
                'zone_overall_mse_scaled': zone_mean_list,
                'zone_counts': zc_mean,
            })
    ...
    averaged_adaptive = average_results_across_seeds(adaptive_result_list)
    averaged_random = average_results_across_seeds(random_results_list)

    # Print summary using averaged random results (baseline)
    if averaged_random:
        print_experiment_summary(averaged_random)
    else:
        print("No averaged random results to summarize.")

    # Save both averaged results and per-seed details to JSON
    ...
    with open(results_file, 'w') as f:
        json.dump({
            ...
            'adaptive_results_avg': averaged_adaptive,
            'random_results_avg': averaged_random,
            'adaptive_per_seed': adaptive_result_list,
            'random_per_seed': random_results_list,
            'comparison': {
                'adaptive_final_mse_avg': (averaged_adaptive[-1]['total_mse'] if averaged_adaptive and averaged_adaptive[-1]['total_mse'] is not None else None),
                'random_final_mse_avg': (averaged_random[-1]['total_mse'] if averaged_random and averaged_random[-1]['total_mse'] is not None else None),
                'adaptive_improvement_pct_avg': ((averaged_adaptive[-1]['total_mse'] / averaged_adaptive[0]['total_mse']) - 1) * 100 if averaged_adaptive and averaged_adaptive[0]['total_mse'] else None,
                'random_improvement_pct_avg': ((averaged_random[-1]['total_mse'] / averaged_random[0]['total_mse']) - 1) * 100 if averaged_random and averaged_random[0]['total_mse'] else None
            }
        }, f, indent=2)

    print(f"\n💾 Aggregated comparison results saved to: {results_file}")

    # ----------------------------
    # Visualization: use averaged results for plots
    # ----------------------------
    try:
        plt.figure(figsize=(12, 8))

        # Prepare x-axes as cumulative total training samples
        x_adaptive = [r['total_samples'] for r in averaged_adaptive]
        x_random = [r['total_samples'] for r in averaged_random]

        # Plot 1: Overall MSE comparison vs total samples
        plt.subplot(2, 2, 1)
        adaptive_mse = [r['total_mse'] for r in averaged_adaptive]
        random_mse = [r['total_mse'] for r in averaged_random]

        plt.plot(x_adaptive, adaptive_mse, 'b-o', label='Adaptive Zone-Based (avg)', linewidth=2)
        plt.plot(x_random, random_mse, 'r-s', label='Random Sampling (avg)', linewidth=2)
        plt.xlabel('Total training samples')
        plt.ylabel('Total MSE')
        plt.title('MSE Evolution: Adaptive vs Random (averaged across seeds)')
        plt.legend()
        plt.grid(True, alpha=0.3)

        # Plot 2: Zone MSE evolution for adaptive approach (x = cumulative samples)
        plt.subplot(2, 2, 2)
        n_iters = len(averaged_adaptive)
        # Determine number of zones dynamically from the averaged results
        n_zones = 0
        for it in averaged_adaptive:
        ...
        for i, it in enumerate(averaged_adaptive):
            vals = it.get('zone_overall_mse_scaled')
            if vals:
        ...
        plt.title('Adaptive (avg): Per-Zone MSE Evolution')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)

        # Plot 3: Sampling strategy visualization - use averaged zone_counts if available
        plt.subplot(2, 2, 3)
        # take zone_counts from last averaged iteration (if present)
        if averaged_adaptive and averaged_adaptive[-1].get('zone_counts'):
            zones = list(range(1, len(averaged_adaptive[-1]['zone_counts']) + 1))
            counts = averaged_adaptive[-1]['zone_counts']
            plt.bar(zones, counts, color='lightblue', edgecolor='navy')
            plt.xlabel('Zone Number')
            plt.ylabel('Avg Times Targeted (per iter)')
            plt.title('Adaptive: Avg Zone Targeting Frequency (last iter)')
            plt.grid(True, alpha=0.3)

        # Plot 4: Improvement comparison vs total samples
        plt.subplot(2, 2, 4)
        if adaptive_mse and adaptive_mse[0] is not None:
            adaptive_improvement = [(adaptive_mse[i] / adaptive_mse[0] - 1) * 100 for i in range(len(adaptive_mse))]
        else:
            adaptive_improvement = []
        if random_mse and random_mse[0] is not None:
            random_improvement = [(random_mse[i] / random_mse[0] - 1) * 100 for i in range(len(random_mse))]
        else:
            random_improvement = []

        plt.plot(x_adaptive, adaptive_improvement, 'b-o', label='Adaptive (avg)', linewidth=2)
        plt.plot(x_random, random_improvement, 'r-s', label='Random (avg)', linewidth=2)
        plt.xlabel('Total training samples')
        plt.ylabel('MSE Change (%)')
        plt.title('Relative Improvement from Baseline (averaged across seeds)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.axhline(y=0, color='k', linestyle='--', alpha=0.5)

        plt.tight_layout()
        comparison_plot = os.path.join('results', 'adaptive_vs_random_comparison_avg.png')
        os.makedirs(os.path.dirname(comparison_plot), exist_ok=True)
        plt.savefig(comparison_plot, dpi=300, bbox_inches='tight')
        plt.close()
        print(f'📊 Aggregated comparison plot saved to: {comparison_plot}')

        # Print summary comparison
        print(f"\n📊 FINAL AGGREGATED COMPARISON SUMMARY:")
        print(f"{'Method':<20} {'Final MSE (avg)':<20} {'Change % (avg)':<15} {'Best Iter (avg)':<15}")
        print(f"{'-'*70}")

        adaptive_final = averaged_adaptive[-1]['total_mse'] if averaged_adaptive else None
        adaptive_change = ((adaptive_final / averaged_adaptive[0]['total_mse']) - 1) * 100 if averaged_adaptive and averaged_adaptive[0]['total_mse'] else None
        random_final = averaged_random[-1]['total_mse'] if averaged_random else None
        random_change = ((random_final / averaged_random[0]['total_mse']) - 1) * 100 if averaged_random and averaged_random[0]['total_mse'] else None

        print(f"{'Adaptive Zone (avg)':<20} {str(adaptive_final):<20} {str(adaptive_change):<15} {'' :<15}")
        print(f"{'Random Sampling (avg)':<20} {str(random_final):<20} {str(random_change):<15} {'' :<15}")

        if adaptive_final is not None and random_final is not None:
            if adaptive_final < random_final:
                improvement = ((random_final / adaptive_final) - 1) * 100
                print(f"\n🎉 Adaptive approach (avg) is {improvement:.1f}% better than random (avg)!")
            else:
                degradation = ((adaptive_final / random_final) - 1) * 100
                print(f"\n⚠️ Adaptive approach (avg) is {degradation:.1f}% worse than random (avg)")

    except Exception as e:
        print('Could not create aggregated comparison plots:', e)

    print(f"\n✅ AGGREGATED ADAPTIVE VS RANDOM EXPERIMENT COMPLETE!")
