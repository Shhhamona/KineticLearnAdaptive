...
def train_and_evaluate_batch_models_using_active_methods(batch_dataset, test_dataset, best_params, seeds=None):
    ...
    models_per_seed, mse_per_output, total_mse = train_initial_models(
        ...
        n_initial_samples=x_train.shape[0], seeds=seeds  # Use all batch samples
    )
...
if __name__ == '__main__':
    ...
    config = {
        ...
        ,
        # Add seeds: either None (default -> [42]), an int, or a list of ints
        'seeds': [i for i in range(42, 46)]
    }
    ...
    batch_models_per_seed, batch_mse_per_output, batch_total_test_mse = train_and_evaluate_batch_models_using_active_methods(
        batch_dataset, test_dataset, best_params, seeds=config.get('seeds')
    )
    ...
    uniform_models_per_seed, uniform_mse_per_output, uniform_total_test_mse = compare_with_uniform_baseline(
        uniform_dataset, test_dataset, best_params,
        batch_size=n_simulations, seeds=config.get('seeds')
    )
    ...
    hybrid_models_per_seed, hybrid_mse_per_output, hybrid_total_test_mse = train_initial_models(
        ...
        n_initial_samples=600, seeds=config.get('seeds')  # Use all 600 samples
    )
    ...
    # -----------------------------
    # ZONE SPLITTING + EVALUATION
    # -----------------------------
    # We will split the augmented training set (current + new) into 6 zones based on
    # Euclidean distance of each sample's raw k-vector to the chosen center k.
    # Assumptions made:
    #  - `uniform_dataset.scaler_output` exists and can inverse_transform scaled y to the
    #    pre-scaled space used when fitting (the pipeline multiplies K by 1e30 before
    #    scaling, so we divide by 1e30 after inverse_transform to get physical units).
    #  - The retrained models (`retrain_models`) are a list of one-model-per-k-output
    #    and accept the scaled input X used in training.

    # Use the reusable function to compute zones and per-zone MSE
    try:
        # For zone evaluation we'll use the first seed's retrained models by default
        ...
        print('\n🔬 ZONE EVALUATION (using scaled Y to define zones)')
        print(f'   Center (scaled Y): {center_y_scaled}')
        print(f'   Zone thresholds (distance in scaled space): {thresholds}')

        for zi in range(6):
            cnt = counts[zi]
            if cnt == 0:
                print(f'   Zone {zi + 1}: EMPTY')
                continue
            per_output_mse = per_output_mse_list[zi]
            overall_mse = overall_mse_list[zi]
            print(f'   Zone {zi + 1:>1} | samples: {cnt:>3} | distance range (scaled): [{thresholds[zi]:.3e}, {thresholds[zi+1]:.3e}]')
            print(f'       Per-output MSE (scaled): {per_output_mse}')
            print(f'       Overall MSE (scaled): {overall_mse:.6e}')
    except Exception as e:
        print('   ⚠️ Could not perform zone evaluation:', e)
