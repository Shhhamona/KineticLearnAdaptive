...
def load_data():
    ...
    results_dir = "results/training_snapshots"

    # Load train_600_Y.txt (scaled k values)
    train_600_file = os.path.join(results_dir, "train_600_Y.txt")
    train_600_y = np.loadtxt(train_600_file)

    # Load test_Y.txt (scaled k values)
    test_file = os.path.join(results_dir, "test_Y.txt")
...
def plot_k_distributions():
    ...
    for i in range(3):
        ...
        ax.hist(
            ...
            bins=15,
            alpha=0.7,
            label=f"Initial Training ({initial_k_scaled.shape[0]})",
            color="green",
            density=True,
        )
        ax.hist(
            ...
            bins=10,
            alpha=0.7,
            label=f"Newly Sampled ({sampled_k_scaled.shape[0]})",
            color="red",
            density=True,
        )
        ax.hist(
            ...
            bins=20,
            alpha=0.7,
            label=f"Test Data ({test_k_scaled.shape[0]})",
            color="blue",
            density=True,
        )
    ...
    # Save the plot using relative path
    output_dir = "results/parameter_distributions"
    os.makedirs(output_dir, exist_ok=True)
    plot_path = os.path.join(output_dir, "k_distributions__comparison.png")
    ...
    for i in range(3):
        ...
        print(
            f"    Range: [{initial_k_scaled[:, i].min():.6f}, {initial_k_scaled[:, i].max():.6f}]"
        )
        ...
        print(
            f"    Range: [{sampled_k_scaled[:, i].min():.6f}, {sampled_k_scaled[:, i].max():.6f}]"
        )
        ...
        print(
            f"    Range: [{test_k_scaled[:, i].min():.6f}, {test_k_scaled[:, i].max():.6f}]"
        )
        ...
        initial_min, initial_max = (
            initial_k_scaled[:, i].min(),
            initial_k_scaled[:, i].max(),
        )
        sampled_min, sampled_max = (
            sampled_k_scaled[:, i].min(),
            sampled_k_scaled[:, i].max(),
        )
        test_min, test_max = test_k_scaled[:, i].min(), test_k_scaled[:, i].max()

        # Check for overlap between training and sampled
        train_sample_overlap = not (
            sampled_max < initial_min or sampled_min > initial_max
        )
        print(
            f"  Training-Sampled overlap: {'✅ YES' if train_sample_overlap else '❌ NO'}"
        )
        ...
        print(
            f"  Sampled data within initial range: {'✅ YES' if within_range else '❌ NO'}"
        )

        # Check if test data is within initial training range
        test_within_train = (test_min >= initial_min) and (test_max <= initial_max)
        print(
            f"  Test data within initial range: {'✅ YES' if test_within_train else '❌ NO'}"
        )

        if not within_range:
            if sampled_min < initial_min:
                print(
                    f"    ⚠️ Sampled minimum ({sampled_min:.6f}) below initial minimum ({initial_min:.6f})"
                )
            if sampled_max > initial_max:
                print(
                    f"    ⚠️ Sampled maximum ({sampled_max:.6f}) above initial maximum ({initial_max:.6f})"
                )
