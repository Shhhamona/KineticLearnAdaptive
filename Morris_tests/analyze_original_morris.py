...
def analyze_good_morris():
    ...
    # Load the good Morris dataC:\Users\rsimoes\Documents\Learning\GPT5Test\data\SampleEfficiency\O2_simple_morris.txt
    good_file = r"C:\Users\rsimoes\Documents\Learning\GPT5Test\data\SampleEfficiency\O2_simple_morris.txt"
...
def test_original_morris_reconstruction():
    ...
    if len(w) == len(param1_unique):
        for i, (grid_val, data_val) in enumerate(zip(w, param1_unique)):
            print(
                f"  Level {i}: Grid={grid_val:.6e}, Data={data_val:.6e}, Match={np.isclose(grid_val, data_val, rtol=1e-6)}"
            )
